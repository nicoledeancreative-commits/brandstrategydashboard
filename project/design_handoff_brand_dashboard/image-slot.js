...
(() => {
  ...
  async function toDataUrl(file, targetW) {
    // SVGs are vector — keep them as-is rather than rasterizing through a
    // canvas (createImageBitmap support for SVG is inconsistent across
    // browsers, and rasterizing would throw away the resolution independence
    // that's the whole point of using an SVG logo).
    ...
    try {
      ...
      const cap = Math.min(MAX_DIM, Math.max(900, Math.round(targetW * 3 * dpr)) || MAX_DIM);
  ...
  const stylesheet =
    ...
    '.ctl{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this._empty.addEventListener('click', () => { if (!this.hasAttribute('readonly')) this._input.click(); });
      ...
      this.addEventListener('dblclick', (e) => {
        if (this.hasAttribute('readonly') || !this.hasAttribute('data-editable') || !this._reframes()) return;
        e.preventDefault();
    ...
    _render() {
      ...
      if (url) {
        ...
        this._empty.style.display = readonly ? 'none' : 'flex';
  ...
  // Expose a minimal read/write/subscribe API so other same-page code (e.g.
  // a host Design Component) can mirror one slot's image into others
  // without asking the user to upload the same file more than once.
  // get()/set() take the slot id and the same {u,s,x,y} (or null) shape
  // used internally; subscribe(fn) calls fn() on every load/drop/clear
  // across ANY slot on the page and returns an unsubscribe function.
